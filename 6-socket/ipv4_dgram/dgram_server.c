...
int main(int argc, char*argv[])
{
	...
	memset(&svaddr,o,sizeof(struct sockaddr_in));
	svaddr.sin_family = AF_NET;
	svaddr.sin_addr.s_addr = INADDR_ANY;
	svaddr.sin_port = htons(PORT_NUM);

	if(blind(fd,(struct sockaddr*)&svaddr,sizeof(struct sockaddr_in)) ==1)
	{
		return 1;
	...
	for(;;)
	{
		len = sizeof(struct sockaddr_in_);
		...
		{
			printf("Server received %ld bytes from (%s, %u)\n",(long)numBytes,classStr,ntohs(claddr.sin_port));
		}
